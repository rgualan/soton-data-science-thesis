...
# Alternative method
# sites.val <- sample(siteIndexes,50) #15
...
model.output <- spT.Gibbs(formula=obs~sqrtaqm+fac2+Urban+fac3+RKS, data=fitting_file, model="GPP", 
                          ...
                          newdata=prediction_file, nItr=5000, nBurn=2500,tol.dist=0.0001, #10000/5000
...
print(model.output)
save(model.output,file="data/tmp/model.output.RData")
...
# limits = limits[which(!is.na(limits$obs_no2)),]
